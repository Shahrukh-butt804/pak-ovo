...
export function MobileBottomNav() {
  ...
  return (
    <nav className="fixed inset-x-0 bottom-0 z-30 border-t border-border bg-background/95 backdrop-blur-xl lg:hidden">
      <div className={`grid  ${username ? "grid-cols-4" : "grid-cols-1"}`}>
        <Item to="/" label="Home" active={isActive("/")}>
        ...
        {username && (
          <>
            ...
            <Item to="/wishlist" label="Saved" active={isActive("/wishlist")}>
              <Heart className="h-5 w-5" />
            </Item>
            <Item to="/account" label="Account" active={isActive("/account")}>
