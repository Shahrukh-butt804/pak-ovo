...
export function Footer() {
  ...
  const { data: categoriesResponse } = useGetAllCategoriesQuery({ refetchOnMountOrArgChange: true ,});
        ...
        <div className="grid gap-10 md:grid-cols-4">
          <div>
            ...
            <p className="mt-4 max-w-xs text-sm text-navy-foreground/70">
              Premium beauty, fashion, home essentials and natural wellness — curated for the modern
              lifestyle.
            </p>
          ...
          {categories && categories?.length > 0 && (
            <FooterCol title="Shop">
              {categories?.map((c:any) => (
                <FooterLink key={c.slug} to="/collections/$slug" params={{ slug: c.slug }}>
                  {c.name || "N/A"}
                </FooterLink>
              ))}
            </FooterCol>
...
function FooterLink({
  ...
  return (
    <li>
      <Link to={to} params={params} className="transition-colors hover:text-gold">
        {children}
      </Link>
    </li>
  );
