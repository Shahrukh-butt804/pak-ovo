...
import { createFileRoute, notFound, useNavigate } from "@/lib/router-compat";
...
function Collection() {
  ...
  const items = useMemo(
    () =>
      (productsResponse?.docs ?? []).map((item: any, index: number) =>
        normalizeProduct(item, index, slug),
      ),
    ...
    <>
        ...
        <div className="container-px mx-auto grid max-w-7xl items-center gap-6 py-8 md:grid-cols-[1fr_220px] md:py-10">
          <div className="min-w-0">
            ...
            <p className="mt-2 max-w-2xl text-sm text-muted-foreground">
              Hand-picked watches — classic dress pieces, modern smartwatches and statement luxury
              timepieces.
            </p>
          </div>
      ...
      <section className="container-px mx-auto max-w-7xl py-10">
        <div className="flex flex-wrap items-center gap-2">
          {!(isLoadingSubCategories || isLoadingCategory) &&
            (subCategories?.docs?.length ?? 0) > 0 && (
              <>
                <button
                  type="button"
