...
import { ArrowRight, Star, Truck, ShieldCheck, RotateCcw, Headphones, Mail, Clock } from "lucide-react";
...
export const Route = createFileRoute("/")({
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Discover premium cosmetics, perfumes, watches, bed sheets, curtains and natural herbs at PakOvo. Free shipping over Rs 5,000." },
      { property: "og:title", content: "PakOvo — Premium Lifestyle Store" },
      { property: "og:description", content: "Beauty, fashion, home essentials and natural wellness in one premium destination." },
...
function Hero() {
  const slides = [
    { img: hero, eyebrow: "New season collection", title: "Elevate your lifestyle", accent: "premium" },
    { img: catPerfumes, eyebrow: "Signature scents", title: "Fragrances you'll love", accent: "luxury" },
      ...
      <div className="container-px mx-auto grid max-w-7xl items-center gap-8 py-8 md:grid-cols-2 md:py-12 lg:py-14">
        <div key={idx} className="animate-fade-up">
          ...
          <p className="mt-4 max-w-md text-sm text-muted-foreground md:text-base">
            Beauty, fashion, home essentials and natural wellness — curated in one place, delivered to your door.
          </p>
          ...
          <div className="mt-6 flex items-center gap-6 text-sm">
            <div className="flex -space-x-2">
              {[1,2,3,4].map(i => (
                <div key={i} className="h-8 w-8 rounded-full border-2 border-background bg-gradient-to-br from-brand to-gold" />
              ))}
            </div>
            <div>
          ...
          <div className="mt-6 flex gap-1.5">
            {slides.map((_, i) => (
              <button
                ...
                className={cn("h-1.5 rounded-full transition-all", i === idx ? "w-8 bg-brand" : "w-4 bg-border")}
              />
            ))}
          </div>
        </div>
...
function Marquee() {
  const items = ["New Season Drops", "Free Shipping Rs 5,000+", "30-Day Returns", "Premium Quality", "Authentic Brands"];
...
function SectionHeader({ eyebrow, title, link, linkText = "View all" }: { eyebrow?: string; title: string; link?: string; linkText?: string }) {
  return (
    <div className="mb-8 flex items-end justify-between gap-4">
      ...
      {link && (
        <Link to={link as any} className="hidden text-sm font-semibold underline-offset-4 hover:underline sm:flex items-center gap-1">
        ...
        {categories.map((c, i) => (
          ...
          >
            ...
            <div className="absolute inset-x-0 bottom-0 p-5 text-navy-foreground">
              <p className="text-xs font-medium uppercase tracking-[0.2em] text-gold">{c.tagline}</p>
...
function NewArrivalsSection() {
  return (
    <section className="container-px mx-auto max-w-7xl py-16 md:py-20">
      ...
      <ProductRow products={newArrivals().slice(0, 8)} />
    </section>
  );
}
...
function BeautyBanner() {
  ...
  return (
    <section className="container-px mx-auto max-w-7xl py-16">
      <Link to="/collections/$slug" params={{ slug: "perfumes" }} className="relative block overflow-hidden rounded-3xl">
        <img key={images[i]} src={images[i]} alt="Beauty & perfume collection" loading="lazy" className="h-[320px] w-full object-cover transition-opacity duration-700 md:h-[400px]" />
        <div className="absolute inset-0 bg-gradient-to-r from-background/95 via-background/40 to-transparent" />
        <div className="absolute inset-y-0 left-0 flex max-w-md flex-col justify-center p-8 md:p-16">
          <p className="text-xs font-semibold uppercase tracking-[0.2em] text-brand">Beauty & Perfume</p>
          <h2 className="mt-2 font-display text-3xl font-bold md:text-5xl">Signature scents,<br />everyday luxury.</h2>
          <p className="mt-4 text-muted-foreground">Discover the season's most coveted fragrances and beauty essentials.</p>
          <Button variant="hero" size="lg" className="mt-6 w-fit">Shop the collection <ArrowRight className="ml-1 h-4 w-4" /></Button>
        </div>
      </Link>
    </section>
  );
  ...
  return (
    <section className="container-px mx-auto max-w-7xl py-16">
      <div className="grid gap-6 md:grid-cols-2">
        <Link to="/collections/$slug" params={{ slug: "bed-sheets" }} className="relative block overflow-hidden rounded-3xl">
          <img src={bannerHome} alt="Bed sheets" loading="lazy" className="h-[420px] w-full object-cover transition-transform duration-700 hover:scale-105" />
          <div className="absolute inset-0 bg-gradient-to-t from-navy/80 to-transparent" />
          <div className="absolute inset-x-0 bottom-0 p-8 text-navy-foreground">
            <p className="text-xs font-semibold uppercase tracking-[0.2em] text-gold">Home Living</p>
        ...
        <Link to="/collections/$slug" params={{ slug: "curtains" }} className="relative block overflow-hidden rounded-3xl">
          <img src={categories[4].image} alt="Curtains" loading="lazy" className="h-[420px] w-full object-cover transition-transform duration-700 hover:scale-105" />
          <div className="absolute inset-0 bg-gradient-to-t from-navy/80 to-transparent" />
          <div className="absolute inset-x-0 bottom-0 p-8 text-navy-foreground">
    ...
    <section className="container-px mx-auto max-w-7xl py-16 md:py-24">
      <div className="grid gap-12 md:grid-cols-[1fr_2fr]">
        <div className="md:py-12">
          <p className="text-xs font-semibold uppercase tracking-[0.2em] text-brand">Nature, gathered</p>
          <h2 className="mt-2 font-display text-4xl font-bold">Herbs & wellness</h2>
          <p className="mt-4 text-muted-foreground">Whole herbs, teas, oils and seeds — sourced for purity, packed with care, and crafted to fit easily into modern routines.</p>
          <Button variant="premium" size="lg" className="mt-6" asChild>
            <Link to="/collections/$slug" params={{ slug: "herbs" }}>Shop herbs <ArrowRight className="ml-1 h-4 w-4" /></Link>
          </Button>
        </div>
        <ProductRow products={herbsProducts} />
      </div>
    </section>
  );
}
      ...
      <div className="container-px mx-auto max-w-7xl py-16 md:py-20">
        <div className="mb-8 flex flex-wrap items-end justify-between gap-4">
          <div>
            ...
            <h2 className="mt-1 font-display text-3xl font-bold md:text-4xl">Flash sale — up to 30% off</h2>
          </div>
          <Countdown />
        </div>
          ...
          {sale.map(p => (
            <Link key={p.id} to="/products/$slug" params={{ slug: p.slug }} className="group">
              ...
              <div className="mt-1 flex items-baseline gap-2">
                ...
                {p.compareAt && <span className="text-xs text-navy-foreground/50 line-through">{formatPrice(p.compareAt)}</span>}
              </div>
            </Link>
          ))}
        </div>
...
function Countdown() {
  ...
  const cell = (n: number, label: string) => (
    <div className="flex flex-col items-center rounded-xl border border-navy-foreground/15 bg-navy-foreground/5 px-3 py-2 min-w-[60px]">
      <span className="font-display text-xl font-bold tabular-nums">{String(n).padStart(2, "0")}</span>
...
function Reviews() {
  const reviews = [
    { name: "Sara Ahmed", role: "Beauty Editor", text: "PakOvo's lipstick is incredibly pigmented and lasts all day. The packaging feels luxurious — easily my go-to brand now.", rating: 5 },
    { name: "James Lee", role: "Verified Buyer", text: "Bought the Heritage Automatic watch — beautiful piece, premium feel, fast shipping. Way better than the price suggests.", rating: 5 },
    { name: "Aisha Khan", role: "Verified Buyer", text: "The Egyptian cotton sheets transformed my bedroom. So soft, true to color, and I love that they wash beautifully.", rating: 5 },
  ];
  return (
    <section className="container-px mx-auto max-w-7xl py-16 md:py-24">
      ...
      <div className="relative overflow-hidden rounded-3xl bg-navy p-8 text-navy-foreground md:p-16">
        ...
        <div className="relative grid items-center gap-8 md:grid-cols-2">
          <div>
            ...
            <h2 className="mt-2 font-display text-3xl font-bold md:text-4xl">Get 10% off your first order.</h2>
            <p className="mt-3 text-navy-foreground/70">Be first to know about new drops, members-only sales and styling tips.</p>
          </div>
          <form onSubmit={(e) => { e.preventDefault(); setSubmitted(true); }} className="flex flex-col gap-3 sm:flex-row">
            ...
            <Button variant="gold" size="lg" type="submit">{submitted ? "Thanks!" : "Subscribe"}</Button>
          </form>
        </div>
      </div>
    </section>
